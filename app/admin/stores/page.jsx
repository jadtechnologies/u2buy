...
export default function AdminStores() {
    ...
    const toggleIsActive = async (storeId) => {
        try {
            ...
            if (data.success) {
                // Update local state to toggle the isActive status
                            ...
                            {store.status === 'approved' && (
                                <div className="flex items-center gap-3 pt-2 flex-wrap">
                                    ...
                                    <label className="relative inline-flex items-center cursor-pointer text-gray-900">
                                        <input type="checkbox" className="sr-only peer" onChange={() => toast.promise(toggleIsActive(store.id), { loading: "Updating...", success: (msg) => msg, error: (err) => err.message })} checked={store.isActive} />
