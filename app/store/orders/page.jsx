                ...
                <div onClick={closeModal} className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50" >
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative">
                        ...
                        <div className="mb-4">
                            ...
                            {(selectedOrder.isCouponUsed || (selectedOrder.coupon && (typeof selectedOrder.coupon === 'string' ? JSON.parse(selectedOrder.coupon).code : selectedOrder.coupon?.code))) && (
                                <p><span className="text-green-700">Coupon:</span> <span className="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full ml-1">
                                    {(() => {
                                        try {
                                            const couponData = typeof selectedOrder.coupon === 'string' ? JSON.parse(selectedOrder.coupon) : selectedOrder.coupon;
                                            return couponData?.code || "Applied";
                                        } catch (e) {
                                            return "Applied";
                                        }
